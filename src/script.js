    ...
    if (response.ok) {
      ...
      if (contentType && contentType.includes("multipart/mixed")) {
        ...
        for (const part of parts) {
          const filename = getFilenameFromContentDisposition(part.headers["content-disposition"]);
          if (filename) {
            ...
            if (filename.endsWith(".mp4")) {
              ...
              await saveFile(new Blob([jsonContent], { type: 'application/json' }), filename);
            }
          }
        }

        console.log("Processing complete. Results saved as separate files.");
        alert("Processing complete. Check your downloads folder for the results.");
      } else {
