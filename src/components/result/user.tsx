...
export function DropdownMenu({
  ...
  const items = [
    ...
    {
      icon: <UserCircleIcon stroke="rgba(0,0,0,0.7)" className="size-[20px]" />,
      name: "About",
      link: "/profile",
    },
    {
  ...
  return (
    <motion.div className="w-full">
      ...
      >
        ...
        <ul
          className={cn(
            "absolute z-[20] max-w-[300px] space-y-3 p-2.5 bg-white border border-black/20 rounded-xl",
          ...
          {items.map(({ icon, name, link }) => (
            <li key={name}>
              <a
                href={link}
                className={cn(
                  "group flex items-center gap-2 rounded-md border border-transparent primary ",
                  itemClassName
                )}
              >
                <span>{icon}</span>
                <span className="flex items-center gap-1 text-sm font-medium primary ease-in-out transition-colors duration-300">
                  {name}
                  <ChevronRightIcon
                    size={12}
